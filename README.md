...
- [TabNet: Attentive Interpretable Tabular Learning](https://arxiv.org/abs/1908.07442) is another model coming out of Google Research which uses Sparse Attention in multiple steps of decision making to model the output. ⚠️ **Note**: The `pytorch-tabnet` dependency is no longer actively maintained (last update: July 2023). Consider using [FTTransformer, GANDALF, or TabTransformer](docs/tabnet_migration_guide.md) for new projects. See [issue #611](https://github.com/pytorch-tabular/pytorch_tabular/issues/611).
