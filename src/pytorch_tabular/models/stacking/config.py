...
class StackingModelConfig(ModelConfig):
    """
    StackingModelConfig is a configuration class for the StackingModel.
    It is used to stack multiple models together.
    Now, CategoryEmbeddingModel, TabNetModel, FTTransformerModel, GatedAdditiveTreeEnsembleModel, DANetModel, AutoIntModel, GANDALFModel, NodeModel are supported.

    Args:
        model_configs (list[ModelConfig]): List of model configs to stack.
    """
    model_configs: list[ModelConfig] = field(
        default_factory=list, metadata={"help": "List of model configs to stack"}
    )
