...
def read_style_df(n_seasons=6, periods=1):
...
target_list = ["block_0", "block_1", "block_2", "block_3", "block_4", "block_5"]
style_df = read_style_df(n_seasons=6, periods=1)
...
style_df = style_df.merge(
    style_df.groupby("mdl")[["retail_price_usa"] + target_list].mean().reset_index(),
...
for t in target_list:
    style_df[f"{t}_predecessor"] = style_df[f"{t}_predecessor"].fillna(
        style_df[f"{t}_predecessor"].mean()
    )
...
sum_target = style_df[target_list].sum(axis=1)
for t in target_list:
    style_df[t] = style_df[t]/sum_target
...
#     activation="SELU",
#     target_range=[
#         (train[t].min().item() * 0.8, train[t].max().item() * 1.2) for t in target_list
#     ],
# )
model_config = NodeConfig(
    ...
    # target_range=[(train[t].min().item()*0.8, train[t].max().item()*1.2) for t in target_list],
)
...
for t in target_list:
    pred_df[f"{t}_prediction"] = pred_df[f"{t}_prediction"]*sum_target
    pred_df[f"{t}"] = pred_df[f"{t}"]*sum_target
pred_df.to_csv("output/temp_5_mlr.csv")
